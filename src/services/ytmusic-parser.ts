...
interface ParsedMusicItem {
  ...
  duration: string;
  resultType: "song" | "artist" | "album";
}

interface TopResultCard {
  title: string;
  thumbnails: { url: string }[];
  videoId: string;
  browseId: string;
  ...
  for (const action of actions) {
    ...
    for (const entry of items) {
      if (entry.musicShelfRenderer || entry.musicShelfContinuation) {
        ...
        continuationToken =
          shelf.continuations?.[0]?.nextContinuationData?.continuation || continuationToken;
      }
...
export function parseTopResultCard(card: any): TopResultCard | null {
  if (!card) return null;

  const title = card.title?.runs?.[0]?.text;
  const subtitleRuns = card.subtitle?.runs || [];
  const thumbnail =
    card.thumbnail?.musicThumbnailRenderer?.thumbnail?.thumbnails?.[0]?.url;
  const videoId =
  ...
  const artists = artistRun
    ? [{ name: artistRun.text, id: artistRun.navigationEndpoint?.browseEndpoint?.browseId }]
    : [];
...
export function parseSuggestions(data: any): string[] {
  ...
  const contents =
    data?.contents?.[0]?.searchSuggestionsSectionRenderer?.contents || data?.contents || [];
...
export function parseMusicItem(item: any): ParsedMusicItem | null {
  if (!item) return null;
...
export function parseTwoRowItem(item: any): TwoRowItem | null {
  if (!item) return null;
