...
export const LastFM = {
  ...
  async getSimilarTracks(title: string, artist: string, limit = "5"): Promise<SimilarTrack[] | { error: string }> {
    const url =
      `https://ws.audioscrobbler.com/2.0/?method=track.getsimilar&artist=${encodeURIComponent(artist)}&track=${encodeURIComponent(title)}&api_key=${API_KEY}&limit=${limit}&format=json`;
...
export async function getTrackInfo(title: string, artist: string): Promise<TrackInfo> {
  try {
    const url =
      `https://ws.audioscrobbler.com/2.0/?method=track.getInfo&artist=${encodeURIComponent(artist)}&track=${encodeURIComponent(title)}&api_key=${API_KEY}&format=json`;
...
export async function getTopArtists(country: string | undefined, limit: number): Promise<any> {
  if (!country) {
    const url =
    ...
    return {
      success: true,
      country: "Global",
      artists: artists.map((a: LastFMArtist) => ({
        name: a.name,
...
export async function getTopTracks(country: string | undefined, limit: number): Promise<any> {
  if (!country) {
    const url =
    ...
    return {
      success: true,
      country: "Global",
      tracks: tracks.map((t: LastFMTrack) => ({
        name: t.name,
        artist: t.artist?.name,
        playcount: t.playcount,
        listeners: t.listeners,
