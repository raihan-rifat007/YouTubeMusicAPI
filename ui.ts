...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .logo{width:44px;height:44px;background:var(--surface2);border-radius:12px;display:flex;align-items:center;justify-content:center;border:1px solid var(--border)}
    .logo svg{width:24px;height:24px;stroke:var(--text);fill:none;stroke-width:1.5}
...
document.querySelectorAll('.nav-btn').forEach(b=>{b.addEventListener('click',function(){document.querySelectorAll('.nav-btn').forEach(x=>x.classList.remove('active'));this.classList.add('active');document.querySelectorAll('.tab').forEach(t=>t.classList.remove('active'));document.getElementById(this.dataset.tab).classList.add('active')})});

document.getElementById('query').addEventListener('keypress',function(e){if(e.key==='Enter')search()});
document.getElementById('searchBtn').addEventListener('click',search);

document.querySelectorAll('.copy-btn').forEach(b=>{b.addEventListener('click',function(e){e.stopPropagation();var url=this.dataset.url;navigator.clipboard.writeText(window.location.origin+url).then(()=>{this.classList.add('copied');setTimeout(()=>this.classList.remove('copied'),2000)})})});

function onYouTubeIframeAPIReady(){yt=new YT.Player('ytplayer',{height:'0',width:'0',host:'https://www.youtube-nocookie.com',playerVars:{autoplay:1,controls:0,disablekb:1,fs:0,modestbranding:1,rel:0},events:{onReady:()=>ready=true,onStateChange:onState,onError:onErr}})}
function onState(e){if(e.data===1){playing=true;document.getElementById('playBtn').innerHTML='<svg viewBox="0 0 24 24"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>';startProgress()}else if(e.data===2||e.data===0){playing=false;document.getElementById('playBtn').innerHTML='<svg viewBox="0 0 24 24"><polygon points="5 3 19 12 5 21 5 3"/></svg>';stopProgress()}if(e.data===0)setTimeout(next,1000)}
...
function onErr(e){if(e.data===150||e.data===101||e.data===100){var s=songs[idx];if(s&&s.fallbackVideoId&&!s.triedFallback){s.triedFallback=true;yt.loadVideoById(s.fallbackVideoId)}else if(s&&!s.triedSearch){s.triedSearch=true;searchYouTube(s.title,s.artists?.[0]?.name||'').then(v=>{if(v)yt.loadVideoById(v)})}}}
async function searchYouTube(title,artist){try{var res=await fetch('/api/yt_search?q='+encodeURIComponent(title+' '+artist+' official')+'&filter=videos');var data=await res.json();return data.results?.find(v=>v.channel?.name&&!v.channel.name.includes('Topic')&&v.id)?.id||null}catch(e){return null}}

async function search(){var q=document.getElementById('query').value.trim();if(!q)return;var f=document.getElementById('filter').value;document.getElementById('searchBtn').disabled=true;document.getElementById('loading').style.display='block';document.getElementById('results').innerHTML='';try{var url='/api/search?q='+encodeURIComponent(q);if(f)url+='&filter='+f;var res=await fetch(url);var data=await res.json();songs=data.results||[];render()}catch(e){songs=[];render()}document.getElementById('searchBtn').disabled=false;document.getElementById('loading').style.display='none'}

function render(){var el=document.getElementById('results');if(!songs.length){el.innerHTML='<div class="empty-state"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><div>No results</div></div>';return}
el.innerHTML=songs.map((s,i)=>{var bid=s.browseId||'';var type=s.resultType||'song';var isPlaylist=bid.startsWith('VL')||type==='playlist';var isAlbum=bid.startsWith('MPRE')&&!isPlaylist;var isArtist=bid.startsWith('UC')||type==='artist';var thumb=s.thumbnails?.[0]?.url||(s.videoId?'https://img.youtube.com/vi/'+s.videoId+'/mqdefault.jpg':'');var click='';if(s.videoId&&(type==='song'||type==='video'))click='play('+i+')';else if(isPlaylist&&bid)click="viewPlaylist('"+bid+"','"+thumb+"','"+encodeURIComponent(s.title||'')+"')";else if(isAlbum&&bid)click="viewAlbum('"+bid+"','"+thumb+"','"+encodeURIComponent(s.title||'')+"')";else if((isArtist||bid)&&!isPlaylist&&!isAlbum)click="viewArtist('"+bid+"','"+thumb+"','"+encodeURIComponent(s.title||'')+"')";var badge=type!=='song'&&type!=='video'&&type!=='playlist'?'<span class="result-badge">'+type+'</span>':'';return'<div class="result-item'+(i===idx?' active':'')+'" onclick="'+click+'"><img class="result-thumb" src="'+thumb+'"><div class="result-info"><div class="result-title">'+esc(s.title||s.name||'Unknown')+badge+'</div><div class="result-artist">'+esc(s.artists?.map(a=>a.name).join(', ')||s.subtitle||'')+'</div></div><div class="result-dur">'+(s.duration||'')+'</div></div>'}).join('')}

function play(i){if(!songs[i]||!ready)return;idx=i;var s=songs[i];document.getElementById('pTitle').textContent=s.title||'Unknown';document.getElementById('pArtist').textContent=s.artists?.map(a=>a.name).join(', ')||'';document.getElementById('pThumb').src=s.thumbnails?.[0]?.url||'https://img.youtube.com/vi/'+s.videoId+'/mqdefault.jpg';document.getElementById('playerBar').classList.add('visible');document.querySelectorAll('.result-item').forEach((el,x)=>el.className=x===i?'result-item active':'result-item');yt.loadVideoById(s.videoId)}
...
async function viewArtist(id,thumbEnc,nameEnc){var searchThumb=decodeURIComponent(thumbEnc||''),searchName=decodeURIComponent(nameEnc||'');document.getElementById('loading').style.display='block';document.getElementById('results').innerHTML='';try{var res=await fetch('/api/artists/'+encodeURIComponent(id));var data=await res.json();var artist=data.artist||data;var tracks=[];if(data.topSongs)tracks.push(...data.topSongs.map(s=>({...s,resultType:'song',videoId:s.videoId,thumbnails:[{url:s.thumbnail}]})));if(data.songs?.results)tracks.push(...data.songs.results.map(s=>({...s,resultType:'song'})));if(data.albums)data.albums.forEach(a=>tracks.push({...a,resultType:'album',browseId:a.browseId,thumbnails:[{url:a.thumbnail}]}));if(data.singles)data.singles.forEach(a=>tracks.push({...a,resultType:'album',browseId:a.browseId,thumbnails:[{url:a.thumbnail}]}));songs=tracks;var thumb=searchThumb||artist.thumbnail||artist.thumbnails?.[0]?.url||'';var name=searchName||artist.name||'Artist';var bio='';try{var bioRes=await fetch('/api/artist/info?artist='+encodeURIComponent(name));var bioData=await bioRes.json();if(bioData.bio)bio=bioData.bio.replace(/<[^>]*>/g,'').split('Read more')[0].trim()}catch(e){}var descHtml=bio?'<div style="color:var(--dim);font-size:0.75rem;margin-top:6px;max-width:500px;line-height:1.4;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden">'+esc(bio)+'</div>':'';var header='<div style="display:flex;align-items:flex-start;gap:16px;padding:16px;margin-bottom:16px;background:var(--surface);border-radius:8px;border:1px solid var(--border)"><img src="'+thumb+'" style="width:64px;height:64px;border-radius:50%;object-fit:cover;background:var(--surface2)"><div style="flex:1"><div style="font-size:1.1rem;font-weight:600">'+esc(name)+'</div><div style="color:var(--muted);font-size:0.8rem">'+(artist.subscribers||'')+'</div>'+descHtml+'<button class="btn" style="margin-top:8px;padding:6px 16px;font-size:0.75rem" onclick="goBack()">Back</button></div></div>';document.getElementById('results').innerHTML=header;render()}catch(e){document.getElementById('results').innerHTML='<div class="empty-state"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><div>Failed to load artist</div></div>'}document.getElementById('loading').style.display='none'}

async function viewAlbum(id,thumbEnc,nameEnc){var searchThumb=decodeURIComponent(thumbEnc||''),searchName=decodeURIComponent(nameEnc||'');document.getElementById('loading').style.display='block';document.getElementById('results').innerHTML='';try{var res=await fetch('/api/albums/'+encodeURIComponent(id));var data=await res.json();var album=data.album||data;var albumThumb=searchThumb||album.thumbnail||album.thumbnails?.[0]?.url||'';var albumName=searchName||album.title||'Album';songs=(data.tracks||[]).map(t=>({...t,resultType:'song',thumbnails:[{url:albumThumb}]}));var artistName=data.artist?.name||album.artists?.map(a=>a.name).join(', ')||'';var header='<div style="display:flex;align-items:center;gap:16px;padding:16px;margin-bottom:16px;background:var(--surface);border-radius:8px;border:1px solid var(--border)"><img src="'+albumThumb+'" style="width:64px;height:64px;border-radius:6px;object-fit:cover;background:var(--surface2)"><div><div style="font-size:1.1rem;font-weight:600">'+esc(albumName)+'</div><div style="color:var(--muted);font-size:0.8rem">'+esc(artistName)+'</div><div style="color:var(--dim);font-size:0.7rem">'+(album.year||'')+' - '+(album.trackCount||songs.length)+' tracks</div><button class="btn" style="margin-top:6px;padding:6px 16px;font-size:0.75rem" onclick="goBack()">Back</button></div></div>';document.getElementById('results').innerHTML=header;render()}catch(e){document.getElementById('results').innerHTML='<div class="empty-state"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><div>Failed to load album</div></div>'}document.getElementById('loading').style.display='none'}

async function viewPlaylist(id,thumbEnc,nameEnc){var searchThumb=decodeURIComponent(thumbEnc||''),searchName=decodeURIComponent(nameEnc||'');document.getElementById('loading').style.display='block';document.getElementById('results').innerHTML='';try{var playlistId=id.startsWith('VL')?id.substring(2):id;var res=await fetch('/api/playlists/'+encodeURIComponent(playlistId));var data=await res.json();var playlistThumb=searchThumb||data.thumbnail||data.thumbnails?.[0]?.url||'';var playlistName=searchName||data.title||'Playlist';var desc=data.description||'';var descHtml=desc?'<div style="color:var(--dim);font-size:0.75rem;margin-top:4px;max-width:500px;line-height:1.4">'+esc(desc)+'</div>':'';songs=(data.tracks||[]).map(t=>({...t,resultType:'song'}));var header='<div style="display:flex;align-items:flex-start;gap:16px;padding:16px;margin-bottom:16px;background:var(--surface);border-radius:8px;border:1px solid var(--border)"><img src="'+playlistThumb+'" style="width:64px;height:64px;border-radius:6px;object-fit:cover;background:var(--surface2)"><div style="flex:1"><div style="font-size:1.1rem;font-weight:600">'+esc(playlistName)+'</div><div style="color:var(--muted);font-size:0.8rem">'+esc(data.author||'')+'</div><div style="color:var(--dim);font-size:0.7rem">'+(data.trackCount||songs.length)+' tracks</div>'+descHtml+'<button class="btn" style="margin-top:6px;padding:6px 16px;font-size:0.75rem" onclick="goBack()">Back</button></div></div>';document.getElementById('results').innerHTML=header;render()}catch(e){document.getElementById('results').innerHTML='<div class="empty-state"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><div>Failed to load playlist</div></div>'}document.getElementById('loading').style.display='none'}

function goBack(){var q=document.getElementById('query').value.trim();if(q)search();else{songs=[];document.getElementById('results').innerHTML='<div class="empty-state"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><div>Search for music</div></div>'}}

var cfg={search:{inputs:[{n:'q',p:'Query',v:'coldplay'}],url:'/api/search'},stream:{inputs:[{n:'id',p:'Video ID',v:'dQw4w9WgXcQ'}],url:'/api/stream'},song:{inputs:[{n:'videoId',p:'Video ID',v:'dQw4w9WgXcQ'}],url:'/api/songs/{videoId}'},album:{inputs:[{n:'browseId',p:'Album ID',v:'MPREb_PvMNqFUp1oW'}],url:'/api/albums/{browseId}'},artist:{inputs:[{n:'browseId',p:'Artist ID',v:'UCIaFw5VBEK8qaW6nRpx_qnw'}],url:'/api/artists/{browseId}'},playlist:{inputs:[{n:'playlistId',p:'Playlist ID',v:'RDCLAK5uy_k'}],url:'/api/playlists/{playlistId}'},chain:{inputs:[{n:'videoId',p:'Video ID',v:'9qnqYL0eNNI'}],url:'/api/chain/{videoId}'},related:{inputs:[{n:'id',p:'Video ID',v:'dQw4w9WgXcQ'}],url:'/api/related/{id}'},radio:{inputs:[{n:'videoId',p:'Video ID',v:'9qnqYL0eNNI'}],url:'/api/radio'},lyrics:{inputs:[{n:'title',p:'Title',v:'Yellow'},{n:'artist',p:'Artist',v:'Coldplay'}],url:'/api/lyrics'},charts:{inputs:[{n:'country',p:'Country',v:'US'}],url:'/api/charts'}};

function updateInputs(){var ep=document.getElementById('endpoint').value,c=cfg[ep];document.getElementById('inputs').innerHTML=c.inputs.map(i=>'<input class="input" id="api_'+i.n+'" placeholder="'+i.p+'" value="'+i.v+'" oninput="updateUrl()">').join('');updateUrl()}
function updateUrl(){var ep=document.getElementById('endpoint').value,c=cfg[ep],url=c.url,params=new URLSearchParams();c.inputs.forEach(i=>{var v=document.getElementById('api_'+i.n)?.value||i.v;if(v){if(url.includes('{'+i.n+'}'))url=url.replace('{'+i.n+'}',encodeURIComponent(v));else params.append(i.n,v)}});var qs=params.toString();if(qs)url+='?'+qs;document.getElementById('urlPreview').textContent='GET '+url}
async function testApi(){var ep=document.getElementById('endpoint').value,c=cfg[ep],url=c.url,params=new URLSearchParams();c.inputs.forEach(i=>{var v=document.getElementById('api_'+i.n)?.value||i.v;if(v){if(url.includes('{'+i.n+'}'))url=url.replace('{'+i.n+'}',encodeURIComponent(v));else params.append(i.n,v)}});var qs=params.toString();if(qs)url+='?'+qs;document.getElementById('response').innerHTML='<pre>Loading...</pre>';try{var res=await fetch(url);var data=await res.json();document.getElementById('response').innerHTML='<pre>'+JSON.stringify(data,null,2)+'</pre>'}catch(e){document.getElementById('response').innerHTML='<pre>Error: '+e.message+'</pre>'}}
updateInputs();
</script>
</body>
</html>`;
